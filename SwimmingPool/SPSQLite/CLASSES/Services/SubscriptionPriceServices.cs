...
namespace SPSQLite.CLASSES
{
    public class SubscriptionPriceServices : ISubscriptionPriceServices
    {
        ...
        {
            IList<ISubscriptionPrice> list =  DatabaseConnection.GetSubscribtionPrice().Select(a => new SubcsriptionPrice { ID=a.Id, NumberOfHours = a.NumberOfHours, Price = a?.Price??0.0 }).ToList<ISubscriptionPrice>();
            return list;
        }
    }
}
